...
**Dream's step 5 has two halves, and the split is the whole point.** 5a is the bot finding a reason to speak tomorrow — at most one, and only on a `high` suitability score. 5b is a date the USER put on the calendar ("下下周六看房", "14 号截止"), where the user supplied both the reason and the day, so no suitability score applies and **any horizon is in scope**. They shared one gate until 2026-08-09, and the effect was that a plan made today for the weekend after next fell out of the skill entirely: step 2's 24h review saw it, and step 5 only ever asked "is there a greeting worth sending tomorrow". Measured at the time on kingsley: 34 of 43 `dream.md` files carried a `## Follow-up` section and the cron store held **zero** `at` jobs — the path ran nightly and self-vetoed every night, so widening the horizon without splitting the gate would have changed nothing.
...
- **`--id` names the event, never the date.** `set-at` upserts by id, so an event raised again on a later night overwrites its own job and a date that moves is corrected by re-filing. A date-derived id (`followup-20260822`) defeats both and silently accumulates duplicates that all fire.
- **`{{CALENDAR}}` covers today ±7d only** (`agent/agent.go:formatCalendar`), and extending it was considered and declined — the system prompt pays for it on every turn of every session. So beyond a week the model computes the date itself, and the skill's compensations are the entire guardrail: read the date off the table inside the window, write the derivation into `dream.md` outside it, and **schedule nothing when the day cannot be pinned**. A reminder on the wrong day is worse than none because nothing traces it back to the turn that filed it.
- **`--task` is the only thing that survives.** A 5b job two weeks out fires long after Tier-2 compression has removed the conversation that motivated it, so the task text has to carry its own context — unlike 5a, which fires while the day is still in the window.
- **`cron list` runs only when there is something to schedule.** It lists every job on the deployment with full task text (~5 KB on kingsley), which is not worth paying on the majority of nights that schedule nothing.
