...
- **Content alongside a dispatch call is now legitimate, and the gate inverted.** dispatch used to hard-reject any assistant content ≥50 runes emitted with the tool_call ("move it into a send body") — correct while `to=user` existed, fatal without it, since content is now the *only* way to reach your own human. The check is gated on `DispatchHost.ContentReachesSomeone()` (`thread/dispatch.go`), which mirrors run.go's intermediate-delivery test exactly: `contentSink` non-zero **and** `Chunkable` **and** not suppressed. Chunkable is load-bearing — an assistant message that also carries tool_calls only delivers on chunkable sinks, so the predicate is "will THIS text actually go out", not "is this session user-facing". Where it reaches someone, report-to-human-in-content + route-with-dispatch is the intended shape; where it doesn't (heartbeat, subagent/fork), the old reject/warn still fires so the text isn't silently dropped. Note `web:`'s defaultSink is not chunkable, so web sessions take the reject path.

**A user-woken turn may not end on a bare dispatch.** When the wake source is user-visible (`CallerKindUser`), the dispatch is solo (so it terminates the turn), `sends` is non-empty, and the assistant message carries no content, the call is rejected before execution — nothing is sent and the turn continues. Without this, a model that hands work to a subagent and stops leaves the person who asked staring at silence, since content is the only channel to one's own human. The rule is narrow on three axes and each one is load-bearing: **solo only** (batched, the turn continues and the model can still speak); **user wakes only** (on a peer or cron wake nobody is waiting — dream's next-day follow-up legitimately drops itself with `dispatch({})`, and narrating every peer exchange to the human is worse than silence); **`ContentReachesSomeone()` only** (demanding text a turn cannot deliver — heartbeat, subagent/fork, or a `web:` session whose defaultSink is not chunkable — would be an unsatisfiable loop). Empty `dispatch({})` is exempt by construction: it returns from the highest-priority branch above, and it is the model explicitly choosing silence rather than forgetting to speak.
