...
- **The context budget trim is the delta's other natural enemy** (`thread/runner.go:trimMessageGroups`). When the estimated request exceeds `contextLoopBudget` it halves the conversation: drops the oldest whole turns (cut at user-message boundaries; system prompt and the final turn always kept), in drop amounts **quantized to multiples of budget/2**. The quantization is the point — any rule that trims to a moving target advances the cut a little every call as the tail grows, rewriting the request head and breaking the prefix DeepEqual each time (measured live: one Tier-2 turn made 4 calls × ~175K prompt tokens at ~0% cache hit, ~44 credits, because the head moved before every call). Quantized, the head moves once per ~budget/2 (~115K on terra) of growth. The trigger line is `(contextWindow − maxTokens) × 0.9`, so `maxTokens` directly positions it relative to the compression tiers: at the old 65,536 default the guard fired barely above Tier-2 (272K window: 185,817 vs 182,000) and on 1M windows well BEFORE it — trimming before compression ever ran. The default is now 16,384 (observed outputs are 700–3,500 tokens), which puts the line at 230,054 on a 272K window — above BOTH compression tiers, a true last resort. If whole turns can't meet the quota (one mega-agentic turn), it falls back to dropping that turn's oldest tool groups under the same quota. Still-over-budget logs a `Warn`, never silent; a prefix break logs `openai continuation dropped: history prefix changed` with the first-diff index. Trimming is ephemeral — the session file is untouched, so Tier-2 eligibility still sees the full size.
