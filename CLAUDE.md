...
- **Cache *write* monitoring**: `provider.Usage.CacheWriteTokens` rides the same path and surfaces as `cacheWriteTokens` in `monitor --metrics`. Only OpenAI's Responses API reports it (`InputTokensDetails.CacheWriteTokens`); everyone else leaves it 0, and `omitempty` keeps it out of their output. It matters because **gpt-5.6 bills a cache write at 1.25x the uncached input rate** — on a cold prefix it is the most expensive line of the request, and it is not visible in `promptTokens - cachedTokens`. Unlike `CachedTokens` it is summed for every provider, not gated on `isCacheUnreliable`: a raw count needs no matched denominator, so an unreliable provider cannot skew it.
