...
// hbStatusEntry is defined in heartbeat_scheduler.go — shared between scheduler and CLI.
...
var heartbeatStatusCmd = &cobra.Command{
	Use:   "status",
	Short: "Show next heartbeat pulse time for all eligible sessions (from live scheduler)",
	RunE: func(_ *cobra.Command, _ []string) error {
		result, err := rpcCall("heartbeat.status", nil)
		if err != nil {
			return fmt.Errorf("heartbeat status: %w", err)
		}
