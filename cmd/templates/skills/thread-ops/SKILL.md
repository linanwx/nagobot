...
Use this after `dispatch(to=subagent|subagent_fork)` to follow up on a child session by the resolved `session_key`.
...
While a subagent you spawned runs long (≥1 min), a background scanner wakes you about once a minute with `source: progress` — a mechanical snapshot of that child (elapsed time, step count, last few tool calls, current tool). The child key is in the body.
