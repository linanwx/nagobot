...
1. **List files**: Run `exec: {{WORKSPACE}}/bin/nagobot list-memory-files` to get memory files needing summaries (max 3, newest first). If none, call `dispatch({})` immediately.
...
3. When all files are processed, call `dispatch({})` to end the turn.
