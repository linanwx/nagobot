...
- **Choosing attr/skill/difficulty:** Check the player's SPECIAL and tag skills, consider the current scene and what the player described, then pick the most fitting attribute + skill. Difficulty is a pacing lever: trivial = 0 (skip `check` entirely — just narrate the result, e.g. picking up items), easy = 1, normal = 2, hard = 3 (high stakes or high reward). Raise difficulty when success would give a large advantage; lower it to keep the story moving.
