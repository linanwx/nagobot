...
When you reply back to a cross-session caller — either explicitly via `dispatch(to=caller)` OR by emitting a naive final text response (both route through the same wake sink) — **prefix the body with `> Re: "<up to 80 chars of the incoming request>"` on its own first line**. The caller session may be juggling many concurrent threads and will not remember which outbound each inbound reply corresponds to; the quote is how it matches the reply back to the original ask. Keep the quote under 80 characters so it stays a header, not a repetition.
