...
- `user` → the channel-user sink of your own session (only valid when your session is user-facing). This is how you reply to the channel user who woke you, AND how you reach your user directly when a non-user source (cron/heartbeat/another session) woke you.
...
**How to pick when replying to whoever woke you:** read the wake YAML — if `caller_session_key` is present, the caller is a peer session → use `caller:session`; if absent AND this session is user-facing, the caller is the channel user → use `to=user`. System sources (cron/heartbeat/compression) have no caller to reply to — use `dispatch({})` to end silently, or `dispatch(to=user)` for user-facing sessions. The tool rejects a mismatched `caller:session` assertion, so a wrong kind is a cheap validation error, not a silent misroute.
...
- `dispatch({sends: [{to: "user", body: "OK"}]})` — plain ack to the channel user who woke you.
